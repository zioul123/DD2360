 ...
 *  i - particle index.
 */
__host__ void h_move_particle(int i, struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param) 
{
    // auxiliary variables
...
int mover_PC(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param)
{
    ...
    for (int i_sub=0; i_sub <  part->n_sub_cycles; i_sub++){
        ...
        for (int i=0; i < part->nop; i++){
            h_move_particle(i, part, field, grd, param);
        }  // end of one particle
    } // end of subcycling
