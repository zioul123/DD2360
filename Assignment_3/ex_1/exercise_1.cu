...
void cpu_grayscale(int width, int height, float *image, float *image_out)
{
    for (int h = 0; h < height; h++)
    {
        ...
        for (int w = 0; w < width; w++)
        {
            ...
            // Convert to grayscale follo* 0.0722fwing the "luminance" model
...
__device__ float gpu_applyFilter(float *image, int stride, float *matrix, int filter_dim)
{
    ...
    //                                                          //
    // Does it make sense to have a separate gpu_applyFilter()? NO :) //
    //////////////////////////////////////////////////////////////
    float pixel = 0.0f;
    for (int h = 0; h < filter_dim; h++)
    {
...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    // In sh_block, there is an extra perimeter, so local_offset_t starts from index 1
    int local_offset_t = threadIdx.y * BLOCK_SIZE_SH + threadIdx.x;

    // Copy over the image to local block
    sh_block[local_offset_t] = image[offset_t];
    // Copy the remnants
    // Bottom row and second bottom row will both copy one pixel, 2-below itself, but make sure it's above the second last row of image
    if ((threadIdx.y == BLOCK_SIZE-1 || threadIdx.y == BLOCK_SIZE-2) && index_y < height-2) { 
        sh_block[local_offset_t + 2 * BLOCK_SIZE_SH] = image[offset_t + 2 * width];
    }
    // Rightmost two rows will copy one pixel 2-right of itself
    if ((threadIdx.x == BLOCK_SIZE-1 || threadIdx.x == BLOCK_SIZE-2) && index_x < width-2) { 
        sh_block[local_offset_t + 2] = image[offset_t + 2];
    }
    // Bottom-right 4 pixels will copy one pixel 2-right 2-below of itself
    if ((threadIdx.y == BLOCK_SIZE-1 || threadIdx.y == BLOCK_SIZE-2) &&
        (threadIdx.x == BLOCK_SIZE-1 || threadIdx.x == BLOCK_SIZE-2) &&
        (index_y < height-2) &&
        index_x < width-2) { 
        sh_block[local_offset_t + 2 * BLOCK_SIZE_SH + 2] = image[offset_t + 2 * width + 2];
    }
    __syncthreads();
    ...
    // Now perform convolution on the image, ensuring bounds are checked
    if (index_x < (width - 2) && index_y < (height - 2))
    {
        image_out[offset] = gpu_applyFilter(&sh_block[local_offset_t],
...
int main(int argc, char **argv)
{
    ...
    // Step 1: Convert to grayscale
    {
        ...
        // cudaDeviceSynchronize(); // Why is there no need for this?
        gettimeofday(&t[1], NULL);
